...
impl<T> Channel<T> {
    ...
    fn discard_all_messages(&self) {
        ...
        let mut head = self.head.index.load(Ordering::Acquire);
        // the channel may be uninitialized, so we have to make sure not to overwrite any sender's attempts to
        // initalize the first block before noticing that the receivers disconnected. late initializations
